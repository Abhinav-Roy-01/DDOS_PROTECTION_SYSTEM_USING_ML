...
def get_click_feed(limit=50):
    """Most recent behavioral telemetry snapshots (mouse/click/keystroke),
    newest first -- backs the dashboard's live telemetry panel."""
    with lock:
        clicks = list(CLICK_LOG)
    return clicks[-limit:][::-1]
