import pickle
import numpy as np
import os

FEATURES = [
    'click_count', 'avg_click_interval', 'click_interval_variance',
    'click_interval_entropy', 'mouse_velocity_variance',
    'max_element_click_rate', 'scroll_events', 'keystroke_count'
]

class MLEngine:
    def __init__(self, model_path='aethercept_model.pkl', scaler_path='aethercept_scaler.pkl'):
        # Model files live in HTTP-LAYER-SHIELD/ML/MODELS/ after the CAPS
        # reorganization -- they used to sit next to server.py. Fixed here
        # rather than moving the .pkl files back, so ML/MODELS/ stays the
        # single source of truth for every trained model in this project.
        api_dir = os.path.dirname(os.path.abspath(__file__))
        models_dir = os.path.join(api_dir, "..", "ML", "MODELS")
        with open(os.path.join(models_dir, model_path), 'rb') as f:
            self.model = pickle.load(f)
        with open(os.path.join(models_dir, scaler_path), 'rb') as f:
            self.scaler = pickle.load(f)
            
    def predict(self, features_dict):
        features = [[features_dict.get(f, 0) for f in FEATURES]]
        scaled = self.scaler.transform(features)
        pred = self.model.predict(scaled)[0]
        score = self.model.decision_function(scaled)[0]
        return {
            'prediction': 'bot' if pred == -1 else 'human',
            'anomaly_score': round(float(score), 4),
            'raw_pred': pred
        }

    def get_features_list(self):
        return FEATURES

# Global instance for easy import
engine = MLEngine()

def predict(features_dict):
    return engine.predict(features_dict)

def get_features_list():
    return engine.get_features_list()
